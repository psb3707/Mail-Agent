...
## 수신 이벤트
수신 패널은 기본 표면과 보라색 주요 버튼을 사용한다. 수신·자동 분류·저장·NEW 단계를 짧은 레이블과 완료 상태로 표시한다. 최근 수신 목록은 메일 제목을 원문 링크로 제공하며 NEW 배지는 작은 보라색 배경으로 통일한다. 열람하면 배지를 제거하고 전체 메일·업무 카드의 개수를 함께 갱신한다. 오류는 수신 패널 안에서 설명하고 이벤트 ID를 유지해 재시도 중복 저장을 방지한다.
