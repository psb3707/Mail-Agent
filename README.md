...
상단에서 시나리오를 선택하고 **새 메일 수신**을 누르면 `수신 → 자동 분류 → 저장 → NEW` 상태가 표시됩니다.

1. **기존 업무 · N_CX 후속 메일**: 계약 완료 메일을 N_CX 업무에 연결합니다.
2. **새 업무 · AURORA 프로젝트**: 독립된 업무를 만들고 메일을 저장합니다. 같은 시나리오의 후속 수신은 같은 업무에 합쳐집니다.
3. **반복 알림 · 보안점검 안내**: 업무와 분리된 알림으로 저장합니다.

최근 수신 메일 제목을 누르면 전체 메일의 원문이 열리고 NEW가 해제됩니다. 업무 타임라인과 Agent의 최근 메일 요약에도 저장된 메일이 반영됩니다. 발표에서는 첫 시나리오 수신 후 “최근 메일 요약해줘”를 질문하고 출처를 열어 확인하세요.

수신은 실제 이메일 공급자 웹훅이 아닌 시연용 이벤트입니다. 원본 데이터는 수정하지 않으며, 저장·읽음 상태는 현재 서버 프로세스 메모리에 유지되어 재시작/재배포 시 초기화됩니다. 단일 프로세스 실행을 전제로 합니다.

API: `POST /inbox/receive` (`event_id`: UUID, `scenario`: existing/new_case/alert), `GET /inbox/events/{event_id}`, `POST /inbox/mails/{mail_id}/read`. 동일 이벤트 ID는 중복 저장하지 않으며 다른 시나리오로 재사용하면 409를 반환합니다.
